  ...
  <main class="container-fluid">
    <div class="row">
      ...
      <div class="col m-4">
        ...
        <!-- Alert -->
        <?php simpanDataPesanan(); ?>
        <!-- Alert -->
        <?php
        if (isset($_GET["no_pesanan"])) {
          $db = dbConnect();
          $no_pesanan = $db->escape_string($_GET["no_pesanan"]);
          if ($dataPesanan = getDataPesananDanDetailPesanan($no_pesanan)) {
        ?>
            <div class="row">
              <div class="col-5">
                <form method="post" action="">
                  <div class="row mb-3">
                    ...
                    <div class="col-auto">
                      <input type="text" class="form-control" id="exampleNoPesanan" name="id_pegawai" value="<?= $dataPesanan['no_pesanan']; ?>" readonly required>
                    </div>
                  </div>
                  <div class="row mb-3">
                    <div class="col-3">
                    ...
                    <div class="col-auto">
                      <input type="Date" class="form-control" id="exampleTanggalLahir" name="tgl" value="<?= $dataPesanan['tgl_pesanan']; ?>" readonly required>
                    </div>
                  </div>
                  <div class="row mb-3">
                    <div class="col-3">
                    ...
                    <div class="col-auto">
                      <input type="text" class="form-control" id="exampleNoMeja" name="no_meja" value="<?= $dataPesanan['no_meja']; ?>" required>
                    </div>
                  </div>
                  <div class="row mb-3">
                    <div class="col">
                      <table class="table table-hover table-borderless">
                        ...
                        <tbody>
                          ...
                          <?php resetDetailPesanan(); ?>
                          <!-- Foreach -->
                          <form action="" method="post">
                            <?php
                            $data = getDataDetailPesanan($no_pesanan);
                            foreach ($data as $detailPesanan) {
                            ?>
                              <tr>
                                <td><?= $detailPesanan['nama_menu']; ?></td>
                                <td>
                                  <input name=<?= "jumlah_pesanan" . $detailPesanan['id_menu']; ?> class="form-control form-control-sm" type="number" value="<?= $detailPesanan['jumlah_pesanan']; ?>" min="1" max="100" style="width: 5em;" id=<?= "jumlah" . $detailPesanan['id_menu']; ?> onchange="return operasi()" autofocus>
                                </td>
                                <td class="text-end">
                                  <input type="text" class="form-control form-control-sm fs-5" readonly value="<?= $detailPesanan['no_pesanan']; ?>" name="no_pesanan" hidden>
                                  <input type="text" class="form-control form-control-sm fs-5" readonly value="<?= $detailPesanan['id_menu']; ?>" name=<?= "id_menu" . $detailPesanan['id_menu']; ?> hidden>
                                  <input type="text" class="form-control form-control-sm fs-5" readonly value="<?= $pegawai_id; ?>" name="id_pegawai" hidden>

                                  <input name="total_bayar" type="text" class="form-control form-control-sm fs-5" readonly value="<?= $detailPesanan['harga_menu']; ?>" id="harga<?= $detailPesanan['id_menu']; ?>" hidden>

                                  <span id="hargaMenu<?= $detailPesanan['id_menu']; ?>"></span>
                        ...
                        <tfoot class="table-light">
                          <tr>
                            ...
                            <td colspan="2" class="text-end">
                              <input type="text" class="form-control-plaintext form-control-sm text-end fs-5" readonly id="total" value="0" name="harga_total">
                            </td>
                          </tr>
                        </tfoot>
                      </table>
                  ...
                  <button type="submit" class="btn font-btn bg--primary font-white me-3" name="btn_simpan">Simpan</button>
          ...
          <?php
            ...
            echo "Nomor Pesanan : $no_pesanan tidak ada. Pengeditan dibatalkan!";
          ?>
        <?php
        } else
          echo "$no_pesanan tidak ada. Pengeditan dibatalkan.";
        ?>
  </main>
